...
app.get("/auth/google/callback123", async (req, res) => {
	const { tokens } = await oAuth2Client.getToken(req.query.code);

	console.log("*** tokens: " + tokens);

	oAuth2Client.getToken(tokens, function (err, tokenInfo) {
		if (err) {
			console.log("*** err: " + err);
...
app.get("/auth/google/callback", function (req, res) {
	try {
		...
		oAuth2Client.getToken(code, async function (err, tokenInfo) {
			...
			console.log("*** - Google access_token: " + tokenInfo.access_token);
...
async function CERP_CREATE_Document(req, res) {
	try {
		//
		iGoogle_oAuthBearer =
			"ya29.a0AWY7CkkMzFrbEDc_U8dkuCZUApCFIcmk4RhlHiDxf9_iijf3gYsga5s57rpmRx4MwUYO8VncTi9nXIabBMSetH4xwCoOxxB-idMPGVBeVrYaNYly7biIFRPwPi8hWe8bGh-shdycH01itf_1T-sVXQDXPwpVaCgYKAb8SARMSFQG1tDrp-7JFEGUEJ5LZKMjuf3fO3g0163";
		...
		let payloads = {
			name: "SHAYMAA_HAFEZ_EBRAHIEM_2024",
			mimeType: "application/vnd.google-apps.document",
		};
